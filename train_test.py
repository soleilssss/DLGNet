...
def train_model(model, criterion, optimizer, train_dataloaders, val_dataloaders, num_epochs, record_path, model_path, best_path):
    ...
    for epoch in np.arange(0, num_epochs) + 1:
        ...
        total_normal = 0.0
        total_polyp = 0.0
        for idx, (input1, input2, labels) in tqdm(enumerate(train_dataloaders), total=len(train_dataloaders)):   # 这边一次取出一个batchsize的东西
            step += 1
            input1, input2, labels = input1.cuda(), input2.cuda(), labels.cuda()
            optimizer.zero_grad()
            _, c1, c2, outputs = model(input1, input2, labels)
            loss1 = criterion(c1, labels)
            loss2 = criterion(c2, labels)
        ...
        with torch.no_grad():  
            ...
            total_normal = 0.0
            total_polyp = 0.0
            for idx, (input1, input2, labels) in tqdm(enumerate(val_dataloaders), total=len(val_dataloaders)):    
                step_val += 1
                input1, input2, labels = input1.cuda(), input2.cuda(), labels.cuda()
                _, c1, c2, outputs = model(input1, input2)
                loss1 = criterion(c1, labels)
                loss2 = criterion(c2, labels)
                loss  = criterion(outputs, labels)
                loss = loss1 + loss2 +loss
            ...
            if acc_class_mean_val > best_val_acc:
                best_val_acc = acc_class_mean_val
...
if __name__ == '__main__':
    ...
    # 定义数据变换及数据集
    train_dataset = BowelDataset(root = r'training set path',transform = transform_train)    # 这是训练集
    print('训练集长度:', len(train_dataset))
    train_dataloaders = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=2)
    val_dataset = BowelDataset(root = r'testing set path',transform = transform_val)       # 这是验证集
    print('验证集长度:', len(val_dataset))
    val_dataloaders = DataLoader(val_dataset, batch_size=40, shuffle=False, num_workers=2)

    # 设置存储路径
    record_path = r'The path where the training results are stored'  # 一些训练结果存储
    model_path = r'The path where the model is stored'  # 间隔存一下模型
    best_path = r'The path where the model with the best result on the validation set is stored'  # 存验证集上最好模型
